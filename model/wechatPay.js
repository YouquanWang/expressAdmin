...
class WechatPay {    /**
    ...
    getUnifiedorderXmlParams(obj){
        var body = '<xml> ' +
            '<appid>'+config.pay.wxappid+'</appid> ' +
            '<body>'+obj.body+'</body> ' +
            '<mch_id>'+config.pay.mch_id+'</mch_id> ' +
    ...
    getSign(signParams){
        ...
        var string = queryString.stringify(newArgs)+'&key='+config.pay.wxpaykey;
        // 生成签名
    ...
    getBrandWCPayParams( obj, callback ){
        ...
        prepay_id_promise.then(function (prepay_id) {
            ...
            var wcPayParams = {
                "appId" : config.pay.wxappid,     //公众号名称，由商户传入
    ...
    getAccessToken(code, cb){
        var that = this;
        var getAccessTokenUrl = "https://api.weixin.qq.com/sns/oauth2/access_token?appid="+config.pay.wxappid+"&secret="+config.pay.wxappsecret+"&code="+code+"&grant_type=authorization_code";
